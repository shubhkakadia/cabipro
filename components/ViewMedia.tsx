...
export default function ViewMedia({
  ...
  // Use a ref to track the last external currentIndex to avoid unnecessary updates
  const lastExternalIndexRef = useRef(currentIndex);

  useEffect(() => {
    if (currentIndex >= 0 && currentIndex < allFiles.length) {
      // Only update if the external index actually changed
  ...
  const convertFileToSelectedFile = useCallback(
    (file: {
      id?: string;
      filename?: string;
      name?: string;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col relative">
        ...
        <div className="flex items-center justify-between p-2 border-t border-slate-200 sticky bottom-4 left-4 right-4">
          ...
          <button
            onClick={() => {
              if (selectedFile.isExisting && selectedFile.url) {
                ...
                // For new files that are actual File objects, create object URL
                ...
                // For Blob objects, use the name from selectedFile
                const fileURL = URL.createObjectURL(selectedFile);
                const a = document.createElement("a");
                a.href = fileURL;
