...
export default function SearchBar() {
  ...
  const handleSelectResult = (key: string, item: item) => {
    let path = null;
    switch (key) {
      case "clients":
        path = item.client_id ? `/app/clients/${item.client_id}` : null;
        break;
      case "employees":
        path = item.employee_id ? `/app/employees/${item.employee_id}` : null;
        break;
      case "projects":
        path = item.project_id ? `/app/projects/${item.project_id}` : null;
        break;
      case "suppliers":
        path = item.supplier_id ? `/app/suppliers/${item.supplier_id}` : null;
        break;
      case "items":
        path = item.item_id ? `/app/inventory/${item.item_id}` : null;
        break;
      default:
        break;
    }
    console.log(path, key, item);
    if (path) {
      setShowSearchDropdown(false);
      router.push(path);
    }
