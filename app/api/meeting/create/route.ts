...
function adelaideLocalToUTC(dateString: string) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get the current user's ID and ensure they're included as a participant
    ...
    const meeting = await prisma.meeting.create({
      data: {
        ...
        participants: {
          connect: allParticipantIds.map((id: string) => ({ id })),
        },
        lots: {
          connect: lot_ids.map((lot_id: string) => ({
            lot_id_organization_id: {
              lot_id: lot_id,
    ...
    try {
      ...
      if (meetingWithDetails) {
        ...
        const lotIdClient =
          meetingWithDetails.lots
            .map((lot: any) => {
              const clientName = lot.project?.client?.name || "Unknown Client";
        ...
        // Send notification (temporarily disabled until lib is available)
