  ...
  try {
    ...
    const updatedMeeting = await prisma.meeting.update({
      ...
      data: {
        ...
        participants: {
          set: participant_ids.map((pid: string) => ({ id: pid })),
        },
        lots: {
          set: lot_ids.map((lid: string) => ({ lot_id: lid })),
        },
      },
      include: {
        participants: {
    ...
    console.error(`Error in PATCH /api/meeting/[id]:`, error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`Error in DELETE /api/meeting/[id]:`, error);
    return NextResponse.json(
      { status: false, message: "Internal server error" },
      { status: 500 },
    );
