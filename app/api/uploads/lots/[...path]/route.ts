...
const TABKIND_TO_ENUM: Record<string, TabKind> = {
...
function getFileKind(mimeType: string | null | undefined): FileKind {
  ...
  try {
    ...
    if (tabKind === "site_measurements" && siteGroup) {
      ...
      if (validSiteGroups.includes(upperSiteGroup as SiteMeasurements)) {
        siteMeasurementGroup = upperSiteGroup as SiteMeasurements;
      }
    }
    ...
    // Find lot by lot_id (string) and verify it belongs to organization
    ...
    // Get or create lot_tab (lot_tab.lot_id references lot.id UUID, not lot.lot_id string)
    let lotTab = await prisma.lot_tab.findFirst({
      where: {
        lot_id: lot.id, // Use lot.id (UUID), not lot.lot_id (string)
        tab: tabKindEnum,
        organization_id: user.organizationId,
      },
    });
