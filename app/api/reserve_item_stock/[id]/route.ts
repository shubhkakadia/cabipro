    ...
    if (quantity !== undefined) {
      ...
      if (qtyDifference > 0) {
        const item = await prisma.item.findUnique({
          where: {
            id: existingReservation.item_id,
      ...
      updatedReservation = await prisma.$transaction(async (tx) => {
        ...
        // Update the reservation
        const updateData: Prisma.reserve_item_stockUpdateInput = {
          quantity: newQty,
        };

        if (mto_id !== undefined) {
          updateData.mto = { connect: { id: mto_id } };
        }
      ...
      // Just update mto_id if quantity not changed
      const updateData: Prisma.reserve_item_stockUpdateInput = {};
      if (mto_id !== undefined) updateData.mto = { connect: { id: mto_id } };
