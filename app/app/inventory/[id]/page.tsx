...
export default function InventoryItemDetailPage() {
  ...
  const fetchHardwareSubCategories = useCallback(async () => {
    try {
      ...
      if (response.data.status && response.data.data) {
        ...
        const subCategories = (
          response.data.data as Array<{ value: string }>
  ...
  const fetchFinishes = useCallback(async () => {
    try {
      ...
      if (axios.isAxiosError(err)) {
        toast.error(err.response?.data?.message || "Error fetching finishes");
      }
      // Fallback to empty array if API fails
      setFinishOptions([]);
    } finally {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !measuringUnitDropdownRef.current.contains(target)
      ) {
        setIsMeasuringUnitDropdownOpen(false);
      }
      if (
        finishDropdownRef.current &&
        !finishDropdownRef.current.contains(target)
      ) {
        setIsFinishDropdownOpen(false);
      }
      if (
        faceDropdownRef.current &&
        !faceDropdownRef.current.contains(target)
      ) {
  ...
  const toggleNotes = (transactionId: string) => {
