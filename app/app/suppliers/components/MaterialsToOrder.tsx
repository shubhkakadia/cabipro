...
const GroupedItemsTable: React.FC<GroupedItemsTableProps> = ({
  items,
  mtoId,
  activeTab,
  onOpenPO,
  ...
  const handleQuantityOrderedChange = (
    ...
    if (timers.has(mtoItemId)) {
      const existingTimer = timers.get(mtoItemId);
      if (existingTimer) clearTimeout(existingTimer);
    }
    const timer = setTimeout(() => {
      saveQuantityOrdered(mtoItemId, nextValue);
    }, 800);
    timers.set(mtoItemId, timer);
  };
  ...
  const groupedItems = useMemo(() => {
    ...
    items.forEach((it: Item) => {
      ...
      const groupItems = groups.get(supplierName);
      if (groupItems) groupItems.push(it);
    });
...
export default function MaterialsToOrder({
  ...
  const [materialsToOrder, setMaterialsToOrder] = useState<MTO[]>([]);
