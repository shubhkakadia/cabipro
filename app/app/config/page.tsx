...
export default function ConfigPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        toast.error(response.data.message || "Failed to create config");
      }
    } catch (err) {
      console.error("Error creating config:", err);
      if (axios.isAxiosError(err)) {
        const errorMessage =
          err.response?.data?.message || "Failed to create config";
        toast.error(errorMessage);
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`/api/config/${selectedItem.id}`, {
        withCredentials: true,
      });
      ...
      if (axios.isAxiosError(err)) {
        const errorMessage =
          err.response?.data?.message || "Failed to delete config";
        toast.error(errorMessage);
      } else {
  ...
  // Open delete modal
  const openDeleteModal = (item: ConfigItem) => {
                ...
                <div className="p-4 shrink-0 border-b border-slate-200">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className="cursor-pointer hover:bg-emerald-700 transition-all duration-200 bg-emerald-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium shadow-sm shrink-0"
                    >
                    ...
                    <table className="min-w-full divide-y divide-slate-200">
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {loading ? (
                          ...
                          paginatedData.map((item: ConfigItem) => (
                            <tr
                              key={item.id}
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  ...
                  className="cursor-pointer px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
